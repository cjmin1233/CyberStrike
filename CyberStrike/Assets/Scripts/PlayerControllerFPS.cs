...
public class PlayerControllerFPS : MonoBehaviour
{
    ...
    [HideInInspector] public float maxVelocityChange = 10f;

    // Internal Variables
    private bool isWalking = false;

    #region Sprint
    [Header("-Sprint-"), Space(10f)]
    ...
    private void Update()
    {
        ...
        if (enableZoom)
        {
            ...
            //    }
            //}
            if (holdToZoom && !isSprinting) isZoomed = playerInput.zoom;
        ...
        if (enableJump && playerInput.jump && isGrounded)
        {
            Jump();
        }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            // Will allow head bob
            if ((Mathf.Abs(targetVelocity.x) >= 0.01f || Mathf.Abs(targetVelocity.z) >= 0.01f) && isGrounded)
            {
                isWalking = true;
            }
            else
...
//// Custom Editor
//#if UNITY_EDITOR
//[CustomEditor(typeof(PlayerControllerFPS)), InitializeOnLoadAttribute]
//public class PlayerControllerFPSEditor : Editor
//{
//    PlayerControllerFPS fpc;
//    SerializedObject SerFPC;

//    private void OnEnable()
//    {
//        fpc = (PlayerControllerFPS)target;
...
//        fpc.maxLookAngle = EditorGUILayout.Slider(new GUIContent("Max Look Angle", "Determines the max and min angle the player camera is able to look."), fpc.maxLookAngle, 0f, 90f);
//        GUI.enabled = true;
...
//            EditorGUILayout.BeginHorizontal();
//            fpc.sprintBarHeightPercent = EditorGUILayout.Slider(new GUIContent("Bar Height", "Determines the height of the sprint bar."), fpc.sprintBarHeightPercent, .001f, .05f);
//            EditorGUILayout.EndHorizontal();
//            EditorGUI.indentLevel--;
//        }
//        GUI.enabled = true;
